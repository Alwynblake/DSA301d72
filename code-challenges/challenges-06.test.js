...
const houseSize = (arr) => {
  ...
  arr.forEach(house => {
    let obj = {};
    obj.house = house.house;
    obj.members = 1 + house.children.length;
    if (house.spouse) obj.members++;
    sizes.push(obj);
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(dead => {
    let obj = {};
    obj.house = dead.house;
    obj.members = 1 + dead.children.length;
    if (dead.spouse) {
      if (!deceasedSpouses.includes(dead.spouse)) {
        obj.members++;
      }
    }
    survivors.push(obj);
