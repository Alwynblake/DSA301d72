...
Write a function named charCode that, given an array of letters as an input, uses map to return a new array where each element is the result of the `charCodeAt` method on
...
const extractStats = (arr) => {
  return arr.map((s) => ({ name: s.stat.name, total: s.effort + s.baseStat }));
};
