...
Write a function named firstLetters that takes in an array of strings and returns an array containing
...
const standardizePhoneNumbers = (arr) => {
  let regex=/\s|-|\(|\)/g;
  return arr.map(x=>x.replace(regex,''));
};
...
const onlyOddChars = (str) => {
  return str.split('').filter((y,i)=>i%2==1).join('')
};
...
const findAnything = (arr, target) => {
  return arr.map(x=>x.match(target))
};
...
We've been testing a new course enrollment system, and we think we have the bugs worked out, but in
the meantime, Brook enrolled himself in a bunch of different classes to test if it was working.
...
Return a two-dimensional array with the same roster, but where anyone whose name includes Brook is
removed from every course.
...
For example, all events on Monday are in the first array, all events on Tuesday are in the second
array, etc.

If an event takes place on multiple days (i.e. "Dancing on Mondays and Tuesdays"), it should appear
in both arrays.
