...
Write a function named validateWord that uses a regular expression pattern to validate that a word is
...
Write a function named hasNumber that uses a regular expression pattern to determine if a string has
one or more letter followed by one or more digit.
...
const findTagNames = (elements) => {
  let regex = /\/\w*/g
  return [].concat(...elements.map(x=>x.match(regex)));
};
